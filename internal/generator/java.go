...
func generateJavaMessage(gen *protogen.Plugin, file *protogen.File, msg *protogen.Message, javaPackage, packageDir string) {
	...
	for _, field := range msg.Fields {
		...
		if fieldMetadata != nil {
			fieldMetadataFilename := filepath.Join(packageDir, msg.GoIdent.GoName+field.GoName+"Metadata.java")
			...
			if !createdJavaMetadataFiles[fieldMetadataFilename] {
				...
				fieldG.P()
				fieldG.P("public final class ", msg.GoIdent.GoName, field.GoName, "Metadata {")
				fieldG.P("    private ", msg.GoIdent.GoName, field.GoName, "Metadata() {} // Prevent instantiation")
				fieldG.P()
				fieldG.P("    public static final Map<String, String> METADATA = new HashMap<>();")
				fieldG.P("    static {")
				// Sort keys for consistent output
				var keys []string
				...
				for _, key := range keys {
					...
					fieldG.P("        METADATA.put(\"", key, "\", \"", value, "\");")
				}
				fieldG.P("    }")
				fieldG.P("}")
			}
