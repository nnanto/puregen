...
func GenerateJavaFile(gen *protogen.Plugin, file *protogen.File, transportNamespace string) {
	...
	if transportNamespace != "" && len(file.Services) > 0 {
		generateGlobalTransportJava(gen, transportNamespace)
	}
	...
	for _, service := range file.Services {
		...
		// Generate client
		generateJavaClient(gen, file, service, javaPackage, packageDir, transportNamespace)
	}
}
...
func generateJavaClient(gen *protogen.Plugin, file *protogen.File, service *protogen.Service, javaPackage, packageDir string, transportNamespace string) {
	serviceName := service.GoName
	...
	if transportNamespace != "" {
		...
		transportPackage := strings.ReplaceAll(transportNamespace, ".", "/")
...
func generateGlobalTransportJava(gen *protogen.Plugin, transportNamespace string) {
	// Only create once per namespace
	if createdTransportNamespacesJava[transportNamespace] {
		return
	}
	createdTransportNamespacesJava[transportNamespace] = true

	// Convert namespace to package directory
	packageDir := strings.ReplaceAll(transportNamespace, ".", "/")
	...
	g.P()
	g.P("package ", transportNamespace, ";")
	g.P()
	g.P("import java.util.*;")
	g.P()
