...
func GeneratePythonFile(gen *protogen.Plugin, file *protogen.File, transportNamespace string) {
	...
	if transportNamespace != "" && len(file.Services) > 0 {
		generateGlobalTransport(gen, transportNamespace)
	}
	...
	if transportNamespace != "" && len(file.Services) > 0 {
		g.P("from ", transportNamespace, " import Transport")
	}
	...
	for _, service := range file.Services {
		generatePythonClient(g, service, transportNamespace)
	}
}
...
func generatePythonClient(g *protogen.GeneratedFile, service *protogen.Service, transportNamespace string) {
	serviceName := service.GoName
...
func generateGlobalTransport(gen *protogen.Plugin, transportNamespace string) {
	// Only create once per namespace
	if createdTransportNamespaces[transportNamespace] {
		return
	}
	createdTransportNamespaces[transportNamespace] = true

	// Create package structure for parent directories
	createTransportPackageStructure(gen, transportNamespace)

	// Create the transport module file
	filename := strings.ReplaceAll(transportNamespace, ".", "/") + "/transport.py"
	g := gen.NewGeneratedFile(filename, "")
	...
	initFilename := strings.ReplaceAll(transportNamespace, ".", "/") + "/__init__.py"
...
func createTransportPackageStructure(gen *protogen.Plugin, transportNamespace string) {
	if !strings.Contains(transportNamespace, ".") {
